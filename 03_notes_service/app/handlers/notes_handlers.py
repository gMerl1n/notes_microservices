...
async def get_note_by_id(note_id: int,
    ...
    note = notes_service.get_note_by_id(async_session=async_session, note_id=note_id)
    if note is None:
...
async def create_note(note: NoteCreateRequest,
                      async_session: AsyncSession = Depends(get_async_session),
                      notes_service: INoteService = Depends(container.get_notes_service)) -> JSONResponse:
    note_id = notes_service.save_note(async_session=async_session, note=note.model_dump())
    if note_id is None:
